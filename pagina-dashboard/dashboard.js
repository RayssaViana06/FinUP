...
exibirDados();
//Aplicando configurações de exibição das seções da dashboard definidas por usuário
...
//Aplica configurações de exibição definidas por usuário
function aplicarConfiguracoes() {
    let configMetas = readConfigMetas();
    let configGastosCategorias = readConfigGastosCategorias();
    let configLimites = readConfigLimites();
    let configContas = readConfigContas();
    let configDesativaGuia = readGuiaDesativado();

    if (configMetas == false) {
        secaoMetas.style.display = 'none';
    }

    if (configGastosCategorias == false) {
        secaoGastosCategorias.style.display = 'none';
    }

    if (configLimites == false) {
        secaoLimitesCategorias.style.display = 'none';
    }

    if (configContas == false) {
        secaoContasAPagar.style.display = 'none';
    }

    if (configDesativaGuia == true) {
        guiaInApp.style.display = 'none';
    }
}
